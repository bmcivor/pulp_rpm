...
class RemoveOldRepodataStep(platform_steps.PluginStep):
    """
    Remove repodata files that are not in repomd and are older then 14 days.
    """
    ...
    def filter_old_repodata(self):
        ...
        repomd_locations = self.parent.repomd_file_context.file_locations

        files_in_dir = [f for f in os.listdir(repodata_dir)
                        if os.path.isfile(os.path.join(repodata_dir, f))]

        # keep files listed in repomd.xml as well as repomd.xml itself
        to_keep = []
        to_keep.append(os.path.basename(repomd))
        to_keep.append(repomd_locations)

        threshold = self.get_config().get(
            'remove_old_repodata_threshold', datetime.timedelta(days=14))
        ...
        for f in files_in_dir:
            ...
            if delta.total_seconds() > threshold.total_seconds() and fname not in to_keep:
                to_remove.append(f)
